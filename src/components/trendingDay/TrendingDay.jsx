...
import { fetchTrendingTvData } from "../../features/counter/trendingTvSlice";
...
function TrendingDay() {
  ...
  const data = useSelector((state) => state.trendingDayData.data);
  const loading = useSelector((state) => state.trendingDayData.loading);
  const genreData = useSelector((state) => state.topRatedGenre.data);
  const [genreIdVSName, setGenreIdVSName] = useState(new Map());

  useEffect(() => {
    dispatch(fetchTrendingTvData());
