...
import { fetchRecommendationData } from "../../features/counter/recommendationSlice";
...
function Recommendation() {
  ...
  const data = useSelector((state) => state.recommendationData.data);
  const loading = useSelector((state) => state.recommendationData.loading);
  const genreData = useSelector((state) => state.topRatedGenre.data);
  const [genreIdVSName, setGenreIdVSName] = useState(new Map());

  useEffect(() => {
    dispatch(fetchRecommendationData(id));
          ...
          <div className={Styles.main__container}>
            ...
            <div className={Styles.main__container__content}>
              {Array.isArray(data.results) && data.results.length > 0 ? (
                ...
                <p>No Recommendation movies available for this item.</p>
              )}
            </div>
          </div>
        </div>
